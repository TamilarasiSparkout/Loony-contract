...
{    
    ...
    //     uint256 totalTaxAmount = (amount * totalTaxPercentage) / 10000; // 3% tax from 100% token

    //     uint256 treasuryTax = (totalTaxAmount * treasuryPercentage) / 100; // 1%  from 3% tax
    //     super._transfer(from, treasuryWallet, treasuryTax);

    //     uint256 donationTax = (totalTaxAmount * donationPercentage) / 100; // 1% from 3% tax
    //     super._transfer(from, donationWallet, donationTax);

    //     uint256 burnTax = (totalTaxAmount * burnPercentage) / 100; // 1% from 3% tax
    //     super._transfer(from, burnWallet, burnTax); // burn

    //     uint256 taxedAmount = treasuryTax + donationTax + burnTax;
    //     uint256 netAmount = amount - taxedAmount;
    //      super._transfer(from, to, netAmount); // remaining tokens to user
